package rakib.hasan.inappupdatesconfig.in_app_update
    ...
    init {
        ...
        appUpdateManager.appUpdateInfo.addOnSuccessListener { info ->
            ...
            if (info.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE) { // UPDATE IS AVAILABLE
                if (info.updatePriority() == 5) { // Priority: 5: Shows Immediate instantly (Recommended for critical updates)
                    if (info.isUpdateTypeAllowed(AppUpdateType.IMMEDIATE)) {
                        startUpdate(info, AppUpdateType.IMMEDIATE)
                    }
                } else if (info.updatePriority() == 4) { // Priority: 4: Shows Flexible after 3 days and Immediate after 5 days
                ...
                } else if (info.updatePriority() == 3) { // Priority: 3: Shows Flexible after 15 days and Immediate after 30 days (Recommended for performance updates)
                    val clientVersionStalenessDays = info.clientVersionStalenessDays()
                ...
                } else if (info.updatePriority() == 2) { // Priority: 2: Shows Flexible after 30 days and Immediate after 90 days (Recommended for minor updates)
                    val clientVersionStalenessDays = info.clientVersionStalenessDays()
                ...
                } else if (info.updatePriority() == 1) { // Priority: 1: Shows Flexible forever
                    // Trigger FLEXIBLE flow
                    startUpdate(info, AppUpdateType.FLEXIBLE)
                } else { // Priority: 0: It does not trigger any update flow
